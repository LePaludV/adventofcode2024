...
func main() {
	...
	for _, v := range input {
		fmt.Println(v)
		// soit fait que augmenté
		// soit fait que descendre
		a := v[0]
		b := v[1]
		isLineOk := 0
		previousValue := a
		...
		testV := v
		k := 0
		// ici on doit bouclé pour tester l'array avec un element en moins

		try := -1
		// tant que try < len(b)-1
		// on test en virant l'elemnt try (si -1 on enlève pas)
		for try < len(v)-1 {
			currentArray := v
			if try != -1 {
				currentArray = slices.Delete(currentArray, try, try+1)
			}
			for i := 1; i < len(v); i++ {
				currentValue := testV[i]
				...
				// 1<abs(previous - new)< 3
				r := math.Abs(float64(previousValue - currentValue))
				if !(r >= 1. && r <= 3.) {
					isOk++
